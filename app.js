...
const STORED_NAME_KEY = 'wc2026_player_name';
const UNDERDOG_RANKING_THRESHOLD = 16; // ranks 16+ are underdogs
...
function restoreLocalPrediction() {
  try {
    ...
    if (data.underdog) state.underdog = data.underdog;
...
function renderUnderdogSelector() {
  const select = document.getElementById('underdogSelect');
  if (!select) return;

  const currentValue = state.underdog || "";
  select.innerHTML = '<option value="">--- Sin underdog ---</option>';

  const allTeams = [];
  GROUP_NAMES.forEach(g => {
    (TEAMS_BY_GROUP[g] || []).forEach(t => {
      if (!allTeams.includes(t.name)) allTeams.push(t.name);
    });
  });

  allTeams.sort().forEach(team => {
    const rank = FIFA_RANKING_TIEBREAK[team] || 999;
    if (rank >= UNDERDOG_RANKING_THRESHOLD) {
      const option = document.createElement('option');
      option.value = team;
  ...
  select.value = currentValue || '';
  state.underdog = select.value;
}

function syncUnderdogSelector() {
  const select = document.getElementById('underdogSelect');
  if (select) select.value = state.underdog;
}
...
function scorePrediction(prediction, results = RESULTS) {
  ...
  if (predUnderdog) {
    const realStages = getKnockoutStageTeamSets(results);
    if (realStages.round32.has(predUnderdog)) score += puntuaciones.underdog.round32;
    if (realStages.round16.has(predUnderdog)) score += puntuaciones.underdog.round16;
    if (realStages.quarterfinals.has(predUnderdog)) score += puntuaciones.underdog.quarterfinals;
    if (realStages.semifinals.has(predUnderdog)) score += puntuaciones.underdog.semifinals;
    if (realStages.finalist.has(predUnderdog)) score += puntuaciones.underdog.finalist;
    if (realStages.champion.has(predUnderdog)) score += puntuaciones.underdog.champion;
  }
...
async function loadLeaderboard() {
  ...
  window.fsOnSnapshot(colRef, (snapshot) => {
    ...
    document.getElementById('leaderboardContent').innerHTML = '<p class="note-text text-red-500">Error al cargar el ranking.</p>';
  });
}
...
function renderLeaderboardList(submissions) {
  ...
  submissions.forEach((entry, index) => {
    ...
    const underdogHtml = entry.underdog
      ? `<span class="leaderboard-underdog">🐎 ${entry.underdog}</span>`
      : '';
    ...
    <div class="scoring-help">
      ...
      <div class="scoring-help-grid">
        ...
        <div class="scoring-help-card">
          ...
          <ul>
            <li>Pasa fase de grupos: <strong>${puntuaciones.underdog.round32} pts</strong></li>
            <li>Llega a octavos: <strong>+${puntuaciones.underdog.round16} pts</strong></li>
            <li>Llega a cuartos: <strong>+${puntuaciones.underdog.quarterfinals} pts</strong></li>
            <li>Llega a semis: <strong>+${puntuaciones.underdog.semifinals} pts</strong></li>
            <li>Llega a la final: <strong>+${puntuaciones.underdog.finalist} pts</strong></li>
            <li>Campeón: <strong>+${puntuaciones.underdog.champion} pts</strong></li>
          </ul>
        </div>
      </div>
...
function renderPredictionReview(entry) {
  const viewer = document.getElementById('predictionViewer');
  const underdog = entry.underdog || entry.prediction?.underdog || '';
  const underdogHtml = underdog ? `<h4>🐎 Underdog</h4><p style="margin:8px 0 16px;font-size:1rem;"><strong>${underdog}</strong></p>` : '';
...
async function checkAlreadySubmitted(name) {
  try {
    ...
    console.error("Error comprobando predicción:", e);
    return false;
  }
}
...
function openWelcomeModal(submitAfter) {
  const modal = document.getElementById('nameModal');
  const input = document.getElementById('playerNameInput');
  const title = modal.querySelector('h3');
  const desc = modal.querySelector('p');
  const cancelBtn = document.getElementById('cancelNameSubmit');
...
function openNameModalForSubmit() {
  const modal = document.getElementById('nameModal');
  const input = document.getElementById('playerNameInput');
  const title = modal.querySelector('h3');
  const desc = modal.querySelector('p');
...
async function confirmWelcomeName() {
  const input = document.getElementById('playerNameInput');
  ...
  const modal = document.getElementById('nameModal');
  if (modal.dataset.submitAfter === 'true') {
    confirmSubmitPrediction(playerName);
  }
}

async function handleNameModalConfirm() {
  const mode = document.getElementById('nameModal').dataset.mode || 'submit';
  if (mode === 'welcome') {
    await confirmWelcomeName();
  } else {
    await confirmSubmitPrediction();
  }
}
...
async function init() {
  ...
  document.getElementById('confirmNameSubmit').addEventListener('click', handleNameModalConfirm);
  ...
  const underdogSelect = document.getElementById('underdogSelect');
  if (underdogSelect) {
    underdogSelect.addEventListener('change', () => {
      state.underdog = underdogSelect.value;
      saveLocalPredictionSoon();
    });
  }

  if (!localStorage.getItem(STORED_NAME_KEY)) {
...
let fbReady = !!window.db;
let initRun = false;

function tryInit() {
  if (domReady && fbReady && !initRun) {
    initRun = true;
    init();
  }
}
